...
def forward_kinematics(hip1, hip2, thigh, foreleg):
...
def inverse_kinematics(desired: np.ndarray, angles_guess: Tuple[float, float], max_iterations: int = 100, learning_rate: int = 0.1) -> Tuple[float, float]:
    ...
    """
    angle1, angle2, angle3, angle4 = angles_guess
    ...
    for _ in range(max_iterations):
        ...
        error = error_function(desired, np.array([angle1, angle2, angle3, angle4]))
        
        # step in a direction to find the error of the 4 angles
        error_angle1 = error_function(desired, np.array([angle1+step1,  angle2,         angle3,         angle4]))
        error_angle2 = error_function(desired, np.array([angle1,        angle2+step2,   angle3,         angle4]))
        error_angle3 = error_function(desired, np.array([angle1,        angle2,         angle3+step3,   angle4]))
        error_angle4 = error_function(desired, np.array([angle1,        angle2,         angle3,         angle4+step4]))
