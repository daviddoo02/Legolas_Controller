...
class Leg:
    ...
    def plot_leg_config(self, joint_positions):
    ...
    def error_function(self, desired_pos: np.ndarray, current: np.ndarray) -> float:
    ...
    def inverse_kinematics(self, desired: np.ndarray, angles_guess: Tuple[float, float], max_iterations: int = 100, learning_rate: int = 0.1) -> Tuple[float, float]:
        ...
        for _ in range(max_iterations):
            ...
            error = self.error_function(desired, np.array(
                [angle1, angle2, angle3, angle4, _]))

            # step in a direction to find the error of the 4 angles
            error_angle1 = self.error_function(desired, np.array(
                [angle1+step1,  angle2,         angle3,         angle4, _]))
            error_angle2 = self.error_function(desired, np.array(
                [angle1,        angle2+step2,   angle3,         angle4, _]))
            error_angle3 = self.error_function(desired, np.array(
                [angle1,        angle2,         angle3+step3,   angle4, _]))
            error_angle4 = self.error_function(desired, np.array(
